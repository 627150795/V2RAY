...
public sealed class HistoryStore
{
    ...
    private static double AbsoluteStability(NodeScore x)
    {
        ...
        var baseScore = .35 * availability + .25 * recentAvailability + .15 * jitter + .25 * recentP95;
        var failurePenalty = x.RecentFailures switch { >= 3 => .35, 2 => .55, 1 => .82, _ => 1d };
        return baseScore * failurePenalty * TailLatencyPenalty(x);
    }
    ...
    private static double RecentPenalty(NodeScore x)
    {
        ...
        return penalty * TailLatencyPenalty(x);
    }
    private static double TailLatencyPenalty(NodeScore x)
    {
        var tail = x.RecentP95Delay > 0 ? x.RecentP95Delay : x.P95Delay;
        if (tail >= 1500) return .60;
        if (tail >= 1000) return .72;
        if (tail >= 700) return .86;
        return 1;
    }
